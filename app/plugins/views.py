...
def app_view_handler(request, plugin_name=None):
    ...
    for mount_point in plugin.app_mount_points():
        view, args, kwargs = try_resolve_url(request, path(r'^/plugins/{}/{}'.format(plugin_name, mount_point.url),
                                                 mount_point.view,
                                                 *mount_point.args,
                                                 **mount_point.kwargs))
        if view:
    ...
    if os.path.exists(plugin.get_path("public")) and plugin.serve_public_assets(request):
        view, args, kwargs = try_resolve_url(request, path('^/plugins/{}/(.*)'.format(plugin_name),
                                                            serve,
...
def api_view_handler(request, plugin_name=None):
    ...
    for mount_point in plugin.api_mount_points():
        view, args, kwargs = try_resolve_url(request, path(r'^/api/plugins/{}/{}'.format(plugin_name, mount_point.url),
                                                 mount_point.view,
                                                 *mount_point.args,
                                                 **mount_point.kwargs))
...
def root_url_patterns():
    ...
    for p in get_active_plugins():
        for mount_point in p.root_mount_points():
            result.append(path(mount_point.url, mount_point.view, *mount_point.args, **mount_point.kwargs))
