...
RUN apt-get -o Acquire::Retries=3 -qq update && \
    apt-get -o Acquire::Retries=3 -qq install -y --no-install-recommends wget curl git g++ clang make cmake postgresql-client && \

    # Install PDAL, letsencrypt, psql, cron
    apt-get -o Acquire::Retries=3 -qq install -y --no-install-recommends binutils pdal certbot gettext tzdata libproj-dev libpq-dev && \
